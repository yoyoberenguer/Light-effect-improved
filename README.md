...
T and have been massively improved using CYTHON and multiprocessing techniques.
...
1) A portion of the screen is taken (corresponding to the size of the light's radial mask),
   often an RGB block of pixels correspond to the area that will be under the light source.
2) Applying changes to the block of RGB values using the pre-defined settings such
...
4) The resulting image is blit onto the background with the additive mode (blending mode) using pygame
   special flag pygame.BLEND_RGBA_ADD
   Note: For 32-bit surface, additive mode will not be required as the surface contains per-pixel alpha
...
- Saturation : Create a saturation effect. You can set a variable between [-1.00 ... 1.0] to adjust the
               saturation level. Below zero the source light will turns slowly to a greyscale and above zero,
               ...
               Both techniques are using C pointer and allocate memory blocks at each function call
               in order to return the tuple hue, saturation, lightness. This imply that each block of memory needs
               to be freed after the function calls. This is done automatically but be aware of that particularity if
